...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        bagData = np.random.choice( range(n), n, replace = True )
        samples = []
        labels = []
    ...
    def fitting(self):
        ...
        for i in range(self.num_trees):
            self.decision_trees[i].tree = self.decision_trees[i].learn(self.bootstraps_datasets[i], self.bootstraps_labels[i])
            print( "Done Training Tree %s, #Nodes: %s" %(str(i+1),str(len(self.decision_trees[i].tree))))
